...
from probe_src.probe_depth_datasets import ProbeShadingDataset, min_max_norm_target, min_max_norm_image, scale_and_norm
...
from probe_src.probe_models import probeLinearDense, probeTwoNonLinearDense
...
def main(args):
    ...
    probe_checkpoints_dir = f"probe_checkpoints/large_syn_dataset_shading/at_step_{at_step}/"
    if not os.path.exists(probe_checkpoints_dir):
        os.makedirs(probe_checkpoints_dir)

    probe_accuracy_dir = f"probe_accuracy/large_syn_dataset_shading/at_step_{at_step}/"
        ...
        for block_ind in range(i_s, i_e):
            ...
            for prompt_ind, seed_num in zip(prompt_indexes, sample_seeds):
                ...
                for layer_ind in range(layer_range):
                    # Save the intermediate activation of the probed layer for this output
                    # They are used as the input to the probing classifier
                    dataset_path = "internal_repres_ester/"
            ...
            for layer_ind in range(layer_range):
                ...
                dataset = ProbeShadingDataset("datasets/images/", 
                ...
                with open("train_indices_500.pkl", "rb") as infile:
                    train_indices = pickle.load(infile)

                with open("test_indices_117.pkl", "rb") as infile:
                ...
                optimizer = torch.optim.Adam(probe.parameters(), lr=1e-2) # lr=1e-3
                ...
                if smoothed.lower() == "yes":
                    ...
                    weights_postfix += "_unsmoothed" + f"_lr0.01_epochs{max_epoch}_500_training_images" # testing a different learning rate
                min_loss = 1e6
                ...
                for epoch in range(1, max_epoch + 1):
                    ...
                    test_results = test_continuous_depth(probe, torch_device, test_dataloader, loss_func=loss_func,
                                                         return_raw_outputs=True, head=None, 
                                                         scheduler=None, verbosity=verbosity,
                                                         smooth_loss=smooth_loss_func, alpha=1)
                    if test_results[0] < min_loss:
                        min_loss = test_results[0]
                        torch.save(probe.state_dict(), f"probe_checkpoints/large_syn_dataset_shading/at_step_{at_step}/regression_probe_{layer}{weights_postfix}.pth")

                # Save the test results at the last epoch for evaluation
                with open(f"probe_accuracy/large_syn_dataset_shading/at_step_{at_step}/saved_test_results_{block}_{block_ind}_layer_{layer_ind}_{postfix}{weights_postfix}.pkl", "wb") as outfile:
                    pickle.dump(test_results[2], outfile)

                # Save the probe model's weights
                torch.save(probe.state_dict(), f"probe_checkpoints/large_syn_dataset_shading/at_step_{at_step}/regression_probe_{layer}_final{weights_postfix}.pth")
                ...
                plt_test_results_continuous_depth(probe,
                                                  ...
                                                  head=None, save_plt=True, norm_output=False, label='Shading',
                                                  save_filename=f"probe_accuracy/large_syn_dataset_shading/at_step_{at_step}/saved_test_results_{block}_{block_ind}_layer_{layer_ind}_{postfix}{weights_postfix}.png")
